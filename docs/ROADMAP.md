...
**수용 기준 (전부 충족)**
- [x] fixtures 의 의도된 CC 29 함수가 `critical` 붉은 건물로 렌더 + 맥동 + 연기
- [x] 의도된 순환 의존 1쌍이 굵은 붉은 튜브로 표시
- [x] 1000+ 파일 분석 ≤ 10s, 재분석 ≤ 1s
- [x] 3000 건물에서 ≥ 55fps
- [x] `unresolved` 비율이 UI 에 노출됨 (해석률 %와 그 의미를 함께)

**Phase 2 실측**

| 항목 | 목표 | 실측 |
|---|---|---|
| 중형 레포 분석 | 1000 파일 ≤ 10s | 1034 파일 **3.2s** |
| 대형 레포 분석 | – | **3041 파일 1.56s** (1946 files/s) |
| 재분석 (캐시) | ≤ 1s | **0.21s** (15배) |
| 프레임레이트 | 3000 건물 ≥ 55fps | **3041 건물 60fps** |
| 테스트 | – | 28개 통과 |

---

## Phase 3 — Agentic Integration ✅ 완료
...
**수용 기준 (전부 충족)**
- [x] "settle 함수를 쪼개줘" → TTFT 2.5s, diff 14.8s 수신
- [x] 같은 파일 재명령 시 TTFT 단축 (프리필 캐시 규약)
- [x] LLM 서버가 없어도 도시 탐색/분석 정상 동작
- [x] 문법이 깨진 생성물은 재파싱 게이트에서 차단, 1회 재시도 후 `agent.error`
- [x] Apply 후 파일 변경, Revert 로 **바이트 단위 복원** 확인
- [x] LLM 실패 시 UI 에 에러 표시, 도시 상태 유지

**Phase 3 실측**

| 항목 | 목표 | 실측 |
|---|---|---|
| 계획 수신 | – | **2.4s** (6단계, json_schema 강제) |
| 첫 토큰 (콜드) | ≤ 5s | **2.5s** |
| 프리필 캐시 (~6k 토큰 접두사) | – | 콜드 4.44s → 웜 **0.51s** (8.8배) |
| diff 완료 | 10~25s | **14.8s** |
| 복잡도 개선 | – | settle() **maxCC 29 → 7** |
| 동작 보존 | – | 20,480개 입력 조합 **불일치 0건** |
| 테스트 | – | 42개 통과 (+ 라이브 LLM 테스트 1개 opt-in) |

`REPOCITY_LIVE_LLM=1` 을 주면 프리필 캐시 회귀 테스트가 실제 엔드포인트로 돈다.

---

## Phase 4 — Interaction ✅ 완료
...
**수용 기준 (전부 충족)**
- [x] Apply → **44ms** 만에 delta 도착, 해당 건물만 애니메이션
- [x] 파일 1개를 3개로 쪼개면 건물 1개가 무너지며 파편이 튀고 3개가 같은 구역에서 솟아오름
- [x] CC 가 낮아지면 색이 전이 (critical 붉은색 → watch 주황, 29 → 7)
- [x] Before/After 토글로 maxCC·LOC·함수 수·등급을 나란히 확인

**Phase 4 실측**

| 항목 | 목표 | 실측 |
|---|---|---|
| Apply → delta 반영 | ≤ 300ms | **44ms** |
| 분석 진행률 이벤트 | – | 2966 파일에 **120건** 발신 |
| 테스트 | – | **56개** 통과 (+ 라이브 LLM 1개 opt-in) |

**Phase 4 에서 고친 설계 결함**: 레이아웃이 메트릭에 의존해, 한 줄 수정에도 29개 건물이
전부 이동했다. 즉 "제자리에서 높이와 색만 바뀐다"는 전제가 성립하지 않았다. 셀 크기를 고정해
레이아웃을 구조의 순수 함수로 만들었다 (설계 결정 6 참조).
...
- ~~**분석 대상 레포 지정 방식**~~ → 해결: 로컬 경로와 git URL 을 모두 받는다 (`sources.py`, 설계 결정 9)
