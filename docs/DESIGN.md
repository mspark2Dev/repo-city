...
| **층 (Floor)** | 함수 하나 | 높이 ∝ 함수 줄 수, 색 = 그 함수의 CC 등급 |
...
**[결정 12] 파일은 하나의 막대가 아니라 함수를 쌓은 탑이다.** 건물 안에 함수마다 슬랩을
하나씩, 소스 순서대로 쌓는다. 높이는 그 함수의 줄 수에, 색은 그 함수의 복잡도에 따른다.
함수 사이의 줄(import, 필드, 클래스 뼈대)은 좁은 안쪽 기둥으로 남아 빈틈이 된다.

건물이 **자기 파일의 단면**이 되는 것이 목적이다. 파일 단위 최대 CC 만 칠하면 `ScheduleService`
같은 2156줄 파일이 그냥 붉은 기둥 하나인데, 층으로 쌓으면 **50층 중 3개 층만 시뻘건 빌딩**이
된다. 실제로 그 파일은 CC 합계 423 중 249가 4개 메서드에 몰려 있다.

슬랩은 등급별 InstancedMesh 에 그대로 합류하므로 **드로우콜은 늘지 않는다**. 936 파일 / 7,895 층
에서 60fps 를 유지한다. 대가는 페이로드로, CityMap 이 1.74MB → 3.56MB 가 된다.

제약: 함수가 1개인 파일은 층을 만들지 않고(그 파일이 곧 그 함수다), 파일당 60층에서 자른다.
그 이상은 어떤 카메라 거리에서도 1픽셀보다 얇다.
