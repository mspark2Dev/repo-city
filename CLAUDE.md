...
## 의존성
필요한 기능을 잘 만들어진 오픈소스 라이브러리로 해결할 수 있고 그 결과 품질이 충분하면,
직접 구현하지 말고 라이브러리를 쓴다. 판단 기준은 취향이 아니라 **측정**이다 — 정확도와
속도를 직접 구현과 비교해 보고 결정하고, 그 근거를 `docs/DESIGN.md` 에 남긴다.
직접 구현을 유지하는 쪽을 택했다면 그 이유도 같이 적는다.
